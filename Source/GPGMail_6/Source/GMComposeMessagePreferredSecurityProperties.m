...
- (GPGKey *)signingKeyFromDraftHeadersIfAvailable {
    @synchronized (self) {
        ...
        GPGKey *signingKey = [self.PGPKeyStatus signingKeyForSender:sender];
...
- (GPGKey *)signingKey {
    @synchronized (self) {
        ...
        // In case only one account is setup in Mail, no signing key
        // is specified.
        // Try to find a key matching the sender address.
        signingKey = [self.PGPKeyStatus signingKeyForAddress:sender];
        if(signingKey != nil) {
            return signingKey;
        }
...
- (NSArray <GPGKey *> *)encryptionKeysForRecipients:(NSArray *)recipients {
    NSMutableArray <GPGKey *> *encryptionKeys = [NSMutableArray new];
    for(NSString *recipient in recipients) {
        id key = [self.keyStatus encryptionKeyForAddress:recipient];
        if([key isKindOfClass:[NSArray class]]) {
            [encryptionKeys addObjectsFromArray:key];
        }
        else {
            [encryptionKeys addObject:key];
        }
    }
