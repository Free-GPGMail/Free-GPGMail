//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <Mail/MFIMAPStoreOperation.h>

@class NSArray;

@interface MFIMAPStoreFlagsOperation : MFIMAPStoreOperation
{
}

- (id)description;	// IMP=0x00000000000fded7
@property(readonly, copy, nonatomic) NSArray *flagsToClear;
@property(readonly, copy, nonatomic) NSArray *flagsToSet;
- (unsigned char)operationType;	// IMP=0x00000000000fdea8
- (id)initWithFlagsToSet:(id)arg1 flagsToClear:(id)arg2 forUids:(id)arg3 inMailbox:(id)arg4;	// IMP=0x00000000000fde1b

@end

